...
# Linux helper to build and install the release APK onto a connected device.
# Mirrors run-release-install.bat with optional automatic ADB-over-Wi-Fi setup.
...
detect_sdk_path() {
  ...
  if sdkmanager_path="$(command -v sdkmanager 2>/dev/null)"; then
    ...
    candidate="$(dirname "$sdkmanager_path")"
...
start_metro() {
  # Metro is typically not required for release but retained for parity with Windows script.
...
main() {
  ...
  if [[ "$SKIP_BUILD" == "1" ]]; then
    ...
    if ! run_gradle_task assembleRelease; then
      exit 1
    fi
  fi
  ...
  if [[ "$SKIP_BUILD" != "1" && ! -f "$RELEASE_APK" ]]; then
    echo "[ERROR] Release APK not found at $RELEASE_APK" >&2
    echo "        (If only app-release-unsigned.apk exists, configure signing first.)" >&2
    exit 1
  fi

  echo "Installing $RELEASE_APK to $chosen_target ..."
  if adb -s "$chosen_target" install -r "$RELEASE_APK"; then
    echo "[OK] Release build installed on $chosen_target."
  else
