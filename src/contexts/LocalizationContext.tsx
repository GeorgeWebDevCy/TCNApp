...
import { Language, TranslationDictionary, TranslationValue, translations } from '../localization/translations';
...
type LocalizationContextValue = {
  ...
  translateError: (value: string | null | undefined) => string | null | undefined;
};

const LocalizationContext = React.createContext<LocalizationContextValue | undefined>(undefined);

const resolveTranslation = (dictionary: TranslationDictionary, key: string): TranslationValue | undefined => {
...
const formatTranslation = (template: string, replacements?: Record<string, string | number>): string => {
  if (!replacements) {
    return template;
  }
...
const errorMessageToKeyMap: Record<string, string> = {
  ...
  'No saved session. Please log in with your password first.': 'errors.noSavedSession',
  'Unable to sign in with PIN.': 'errors.pinLogin',
  'Biometric authentication is not available on this device.': 'errors.biometricsUnavailable',
  ...
  'Please log in with your username and password before creating a PIN.': 'errors.loginBeforePinCreation',
  'You must log in with your password before setting a PIN.': 'errors.loginBeforePinSetting',
  'Please log in with your username and password before changing your PIN.': 'errors.loginBeforePinChange',
  'PIN must contain at least 4 digits.': 'errors.pinLength',
  'Biometric authentication is not configured.': 'errors.biometricsNotConfigured',
...
export const LocalizationProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const translate = useCallback(
    (key: string, options?: TranslateOptions) => {
      ...
      const value = resolveTranslation(currentDictionary, key) ?? resolveTranslation(fallbackDictionary, key);
  ...
  const translateError = useCallback<LocalizationContextValue['translateError']>(
    (value) => {
      if (!value) {
        return value;
      }

      const key = errorMessageToKeyMap[value] ?? (value.startsWith('errors.') ? value : undefined);
      if (key) {
